        ...
        {["Tipo", "Fecha", "Confirmar"].map((s, i) => (
          <div key={s} className="flex items-center gap-2 sm:gap-3 flex-1">
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="text-xs sm:text-sm font-medium hidden sm:inline" style={{ color: i === 0 ? C.text : C.textMuted }}>{s}</span>
            </div>
        ...
        {options.map((o) => {
          ...
          return (
            <button
              ...
              className="flex items-center gap-3 sm:items-start sm:gap-4 p-4 sm:p-5 rounded-2xl border text-left transition-all duration-200 cursor-pointer"
              style={{
            ...
            >
              ...
              >
                <o.icon size={18} color={selected ? "#fff" : C.brand} />
              </div>
              <div>
                <p className="text-sm sm:text-[15px] font-semibold mb-0.5" style={{ color: selected ? C.brand : C.text }}>{o.label}</p>
                <p className="text-xs sm:text-sm" style={{ color: C.textSecondary }}>{o.desc}</p>
              </div>
            </button>
          );
        })}
